import torch
import math
from extern.pytracking.libs import fourier
from extern.pytracking.libs import complex
...
def label_function(sz: torch.Tensor, sigma: torch.Tensor, half: bool = True):
    return gauss_fourier(sz[0].item(), sigma[0].item()).reshape(1, 1, -1, 1) * gauss_fourier(sz[1].item(), sigma[1].item(), half).reshape(1, 1, 1, -1)
