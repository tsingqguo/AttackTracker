...
from extern.siamdw.lib.utils.utils import make_scale_pyramid
from math import cos, sin, pi
import os
import cv2


class OIMAttackerSiamDW():
    ...
    def attack(self, tracker, img, prev_perts=None, weights=None,APTS=False, OPTICAL_FLOW=False, ADAPT=False, Enable_same_prev=True):
        ...
        # generate original x_crops
        state = tracker.state
        p = state['p']
        avg_chans = state['avg_chans']
        target_pos = state['target_pos']
        scaled_instance = p.s_x * p.scales
        x_crop = make_scale_pyramid(img, target_pos, scaled_instance, p.instance_size, avg_chans)

        # generate adv labels
        if self.type == 'UA':
            adv_cls, same_prev= self.ua_label(img, tracker)
            #adv_cls = adv_cls.long()
        elif self.type == 'TA':
            adv_cls, same_prev = self.ta_label(img, tracker)
            #adv_cls = adv_cls.long()
        ...
        if prev_perts is None or (same_prev == False and Enable_same_prev == True):
            if cfg.CUDA:
                prev_perts = torch.zeros(x_crop.size()).unsqueeze(0).cuda()
                weights = torch.ones(x_crop.size()).unsqueeze(0).cuda()#torch.ones(1).cuda()
            else:
                prev_perts = torch.zeros(x_crop.size()).unsqueeze(0)
                weights = torch.ones(x_crop.size()).unsqueeze(0)#torch.ones(1)
        else:
            if APTS == False:
        ...
        while m < max_iteration:
            data = {
                'template_zf': tracker.state['net'].zf.detach(),
        ...
        if cfg.CUDA:
            prev_perts = torch.cat((prev_perts, pert.unsqueeze(0)), 0).cuda()
            weights = torch.cat((weights, torch.ones(x_crop.size()).unsqueeze(0).cuda()),0).cuda()
        else:
            prev_perts = torch.cat((prev_perts, pert.unsqueeze(0)), 0)
            weights = torch.cat((weights, torch.ones(x_crop.size()).unsqueeze(0)), 0)
    ...
    def ua_label(self, img, tracker):

        tracker.track(img)
        score = tracker.state['score']
        score = score.permute(2,0,1).unsqueeze(1)
        adv_cls = torch.zeros_like(score)
        #adv_cls[score>(score.max()*0.98)]=1.0

        obj_pos = tracker.state['target_pos']
        size = tracker.state['target_sz']

        context_size = tracker.state['p'].s_x

        same_prev = False
        # validate delta meets the requirement of obj
        if self.prev_delta is not None:
            diff_pos = np.abs(self.prev_delta - obj_pos)
            if (size[0] // 2 < diff_pos[0] and diff_pos[0] < context_size// 2) \
                    and (size[1] // 2 < diff_pos[1] and diff_pos[1] < context_size // 2):
                delta = self.prev_delta
                same_prev = True
            else:
                delta = []
                if size[0]>context_size:
                    min_delta1 = context_size // 2
                    max_delta1 = size[0] // 2
                else:
                    min_delta1 = size[0] // 2
                    max_delta1 = context_size // 2

                if size[1]>context_size:
                    min_delta2 = context_size // 2
                    max_delta2 = size[1] // 2
                else:
                    min_delta2 = size[1] // 2
                    max_delta2 = context_size // 2
                delta.append(random.choice((1, -1)) * random.randint(min_delta1, max_delta1))
                delta.append(random.choice((1, -1)) * random.randint(min_delta2, max_delta2))
                delta = obj_pos + np.array(delta)
                self.prev_delta = delta
        else:
            delta = []
            delta.append(random.choice((1, -1)) * random.randint(size[0] // 2, context_size // 2 - size[0] // 2))
            delta.append(random.choice((1, -1)) * random.randint(size[1] // 2, context_size// 2 - size[1] // 2))
            delta = obj_pos + np.array(delta)
            self.prev_delta = delta

        desired_pos = context_size / 2 + delta
        upbound_pos = [context_size,context_size]
        ...
        if cfg.ATTACKER.EVAL:
            desired_pos_abs = obj_pos + delta
            tpos = []
            tpos.append(desired_pos_abs[0])
            tpos.append(desired_pos_abs[1])
            self.target_traj.append(tpos)

        desired_pos_ = np.clip(desired_pos*(score.size()[-1]/context_size),0,score.size()[-1]-1)

        #print("adv_cls",adv_cls.size(),"desired_pos_",desired_pos_)
        adv_cls[:,:, int(desired_pos_[1]), int(desired_pos_[0])] = 1

        self.target_pos = desired_pos

        return adv_cls,same_prev

    def ta_label(self, img, tracker):
        ...
        center_pos = tracker.state['target_pos']
    ...
    def oim_once(self, tracker, data):
        ...
        #print("pert", pert.size())
        #print("pert_sum",pert_sum.size())
        #print("prev_perts",prev_perts.size())

        x_crops = search + pert + pert_sum #.view(1, prev_perts.shape[1], prev_perts.shape[2], prev_perts.shape[3],prev_perts.shape[4])
        xf = net.feature_extractor(x_crops)
        cls = net.connector(zf, xf)

        def _cls_loss(pred, label, select):
        ...
        def _weighted_BCE(pred, label):
            pred = pred.view(-1)
            label = label.view(-1)
            pos = label.data.eq(1).nonzero().squeeze().cuda()
            neg = label.data.eq(0).nonzero().squeeze().cuda()

            loss_pos = _cls_loss(pred, label, pos)
            loss_neg = _cls_loss(pred, label, neg)

            return loss_pos * 0.5 + loss_neg * 0.5

        cls_loss = _weighted_BCE(cls, adv_cls)

        # regularization loss
        if cfg.CUDA:
            c_prev_perts = torch.cat((prev_perts, pert.unsqueeze(0)), 0).cuda()
        else:
            c_prev_perts = torch.cat((prev_perts, pert.unsqueeze(0)), 0)


        t_prev_perts = c_prev_perts.view(c_prev_perts.shape[0]*c_prev_perts.shape[1]*c_prev_perts.shape[2],
                                         c_prev_perts.shape[3]*c_prev_perts.shape[4])
        ...
        if self.norm_type == 'L_inf':
            ...
            new_pert = adv_x - search-pert_sum
            new_pert = torch.clamp(new_pert, -self.inta, self.inta)
        elif self.norm_type == 'L_1':
            x_grad = x_grad / torch.norm(x_grad, 1)
            adv_x = adv_x + pert + pert_sum  + self.eplison * x_grad
            new_pert = adv_x - search-pert_sum
            new_pert = torch.clamp(new_pert / torch.norm(new_pert, 1), -self.inta, self.inta)
        elif self.norm_type == 'L_2':
            x_grad = x_grad / torch.norm(x_grad, 2)
            adv_x = adv_x + pert + pert_sum  + self.eplison * x_grad
            new_pert = adv_x - search-pert_sum
            new_pert = torch.clamp(new_pert / torch.norm(new_pert, 2), -self.inta, self.inta)

        p_search = search + new_pert + pert_sum
        p_search = torch.clamp(p_search, 0, 255)
        new_pert = p_search - search - pert_sum

        return new_pert, total_loss, cls
