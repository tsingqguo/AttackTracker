...
from pysot.tracker.base_tracker import pyBaseTracker
from pysot.core.config import cfg
from extern.pytracking.libs import dcf, TensorList
from extern.pytracking.features.preprocessing import numpy_to_torch
from extern.pytracking.utils.plotting import show_tensor
from extern.pytracking.features import augmentation
from pysot.tracker.eco_optim import FilterOptim, FactorizedConvProblem
from extern.pytracking.libs.optimization import GaussNewtonCG
from extern.pytracking.libs import complex, fourier, TensorList
from extern.pytracking.libs.tensorlist import tensor_operation
from extern.pytracking.utils.visdom import Visdom

class ECOTracker(pyBaseTracker):
    ...
    def init(self, im_narray, bbox):
        ...
        self.visdom = None
        if cfg.PYTRACKING.SHOW and self.visdom is None:
            self.visdom = Visdom(self.params.debug, {'handler': self.visdom_ui_handler, 'win_id':  'Tracking'},\
                    {'server':'127.0.0.1','port':8097})

        # Chack if image is color
        self.params.features.set_is_color(im_narray.shape[2] == 3)
        ...
        self.center_pos = self.pos.numpy()
        self.size = self.target_sz.numpy()
        # Set search area
    ...
    def track(self, image, x_crop=None,ispert=False):
