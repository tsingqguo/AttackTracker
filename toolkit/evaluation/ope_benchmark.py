...
class OPEBenchmark:
    ...
    def eval_success(self, eval_trackers=None):
        ...
        for tracker_name in eval_trackers:
            ...
            for video in self.dataset:
                ...
                #print('tracker_name:'+tracker_name+',video name:'+video.name)
    ...
    def eval_map(self, eval_trackers=None):
        """
        Args:
            eval_trackers: list of tracker name or single tracker name
        Return:
        ...
        for tracker_name in eval_trackers:
            ...
            for video in self.dataset:
                if tracker_name not in video.pred_trajs:
                    map_res = video.load_map(self.dataset.tracker_path,
                            tracker_name, False)
                    map_res = np.array(map_res)
                if hasattr(video, 'absent'):
                    map_res = map_res[video.absent == 1]
                map_ret_[video.name] = np.mean(map_res[1:])
            map_ret[tracker_name] = map_ret_
        return map_ret

    def eval_p(self, eval_trackers=None):
        """
        Args:
            eval_trackers: list of tracker name or single tracker name
        Return:
        ...
        for tracker_name in eval_trackers:
            ...
            for video in self.dataset:
                if tracker_name not in video.pred_trajs:
                    map_res = video.load_p(self.dataset.tracker_path,
                            tracker_name, False)
                    map_res = np.array(map_res)
                if hasattr(video, 'absent'):
                    map_res = map_res[video.absent == 1]
    ...
    def eval_AvrIterNum(self, eval_trackers=None):
        """
        Args:
            eval_trackers: list of tracker name or single tracker name
        Return:
        ...
        for tracker_name in eval_trackers:
            ...
            for video in self.dataset:
                if tracker_name not in video.pred_trajs:
                    iternum_ = video.load_iter(self.dataset.tracker_path,
                            tracker_name, False)
                    iternum_ = np.array(iternum_)
                if hasattr(video, 'absent'):
                    iternum_ = iternum_[video.absent == 1]
                iter_ret_[video.name] = np.mean(iternum_[1:])
            iternum_ret[tracker_name] = iter_ret_
        return iternum_ret

    def eval_time(self, eval_trackers=None):
        """
        Args:
            eval_trackers: list of tracker name or single tracker name
        Return:
        ...
        for tracker_name in eval_trackers:
            ...
            for video in self.dataset:
                if tracker_name not in video.pred_trajs:
                    time_per = video.load_time(self.dataset.tracker_path,
                            tracker_name, False)
                    time_per = np.array(time_per)
                if hasattr(video, 'absent'):
                    time_per = time_per[video.absent == 1]
                #avgtime_pervid[video.name] = np.mean(time_per)*1000.
                print("len:",time_per.shape[0],"time:",np.sum(time_per))
                avgtime_pervid[video.name] = time_per.shape[0]/np.sum(time_per)
            avgtimes_[tracker_name] = avgtime_pervid

        return avgtimes_

    def eval_ta_precision(self, eval_trackers=None):
        """
        Args:
            eval_trackers: list of tracker name or single tracker name
        Return:
        ...
        for tracker_name in eval_trackers:
            precision_ret_ = {}
            for video in self.dataset:
                ta_center = np.array(video.ta_traj)
                if tracker_name not in video.pred_trajs:
                    tracker_traj = video.load_tracker(self.dataset.tracker_path,
                            tracker_name, False)
                    tracker_traj = np.array(tracker_traj)
                    ...
                    tracker_traj = np.array(video.pred_trajs[tracker_name])
                n_frame = len(ta_center)
                if hasattr(video, 'absent'):
                    ta_center = ta_center[video.absent == 1]
                    tracker_traj = tracker_traj[video.absent == 1]
                tracker_center = self.convert_bb_to_center(tracker_traj)
                thresholds = np.arange(0, 51, 1)
                precision_ret_[video.name] = success_error(ta_center, tracker_center,
                        thresholds, n_frame)
            precision_ret[tracker_name] = precision_ret_
        return precision_ret
    ...
    def show_result_attack(self, success_ret, precision_ret=None,
            MAP_ret=None, TIME_ret=None,show_video_level=False, helight_threshold=0.6):
        """pretty print result
        Args:
            result: returned dict from function eval
        """
        ...
        tracker_name_len = max((max([len(x) for x in success_ret.keys()])+2), 12)
        header = ("|{:^"+str(tracker_name_len)+"}|{:^9}|{:^16}|{:^11}|").format(
                "Tracker name", "Time", "MAP", "Precision")
        formatter = "|{:^"+str(tracker_name_len)+"}|{:^9.3f}|{:^16.3f}|{:^11.3f}|"
        print('-'*len(header))
        print(header)
        print('-'*len(header))
        for tracker_name in tracker_names:
            ...
            if MAP_ret is not None:
                MAP = np.mean(list(MAP_ret[tracker_name].values()),
                        axis=0)
            else:
                MAP = 0

            if TIME_ret is not None:
                TIME = np.mean(list(TIME_ret[tracker_name].values()),
                            axis=0)
            else:
                TIME = 0

            print(formatter.format(tracker_name, TIME, MAP, precision))
        print('-'*len(header))
