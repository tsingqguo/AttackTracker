...
parser.add_argument('--interval', type=int,
                    default=-1,
        help='-2 : random attack with 10% probability; -1:adapt,>0:attack with fixed interval, 1000:only use the perturation from the first frame')
...
def main():
    ...
    # build attacker
    attacker = build_attacker(args.att_method,args.att_type,args.max_num,args.apts_num,reg_type=args.reg_type)
    ...
    for v_idx, video in enumerate(dataset):
        ...
        save_name = args.name_suffix+save_name
        ...
        for idx, (img, gt_bbox) in enumerate(video):
            ...
            if idx == 0:
                ...
                # for random attack
                if args.interval == -2:
                    max_att_num = int(np.round(len(video.img_names) * 0.1))
                    slt_idxes = np.random.choice(len(video.img_names),max_att_num)
                    enable_atts = np.zeros((len(video.img_names),1),dtype=int)
                    enable_atts[slt_idxes]=1
                ...
                # adative attack
                    ...
                    APTS, OPTICAL_FLOW, ADAPT = args.apts, args.opt_flow, False
                    if enable_atts[idx] ==1 and enable_atts[0:idx].sum()<max_att_num:
                        prev_perts=None
                ...
                x_crop, pert_true, prev_perts, img = attacker.attack(tracker, img, t_prev_perts, APTS, OPTICAL_FLOW,ADAPT,Enable_same_prev=False)
