...
from pysot.attacker.oim_attacker_cross_img import CROSSOIMAttacker
...
def main():
    ...
    # OPE tracking
    for v_idx, video in enumerate(tqdm(dataset)):
        if args.video != '':
        ...
        if (args.vis or cfg.ATTACKER.SAVE_VIDEO):
            vid_root = os.path.join('results', args.dataset, 'vis'+save_name)
        ...
        for idx, (img, gt_bbox) in enumerate(video):
            ...
            if idx == 0:
                ...
                if args.name_suffix=="nogtbox":
                    print(args.name_suffix)
                    trackerC.init(img, gt_bbox_,ifdetect=True)
                elif args.name_suffix=="nogtbox_detect":
                    print(args.name_suffix)
                    checkpoint = '/home/guoqing/projects/AttackTrack/extern/ssd_detect/model/checkpoint_ssd300.pth.tar'
                    checkpoint = torch.load(checkpoint)
                    trackerC.init(img, gt_bbox_,ifdetect=True,using_detect=True,checkpoint=checkpoint)
                else:
                ...
                if args.att_type=='TA' and (idx==1 or idx%10==0):
                    trackerC.center_pos = tracker.center_pos
                    trackerC.size = tracker.size

                _, pert_true, prev_perts, adv_img = attacker.attack(trackerC, img, t_prev_perts, APTS, OPTICAL_FLOW,ADAPT)

                #
                # import visdom
                # vis = visdom.Visdom()
                # adv_img_disp = adv_img.detach().cpu().byte().numpy().squeeze(0).transpose(1, 2, 0)
                # adv_img_disp = cv2.cvtColor(adv_img_disp, cv2.COLOR_BGR2RGB)
                # vis.image(adv_img_disp.transpose(2, 0, 1))
                #
                # img_disp = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
                # vis.image(img_disp.transpose(2, 0, 1))
                #
                # diff_img = np.clip(adv_img_disp-img_disp,0,255.0)
                # diff_img =255.*(diff_img-diff_img.min())/(diff_img.max()-diff_img.min())
                # vis.image(diff_img.transpose(2, 0, 1))
                ...
                adv_img = adv_img.squeeze(0).permute(1,2,0).detach().cpu().numpy()
                outputs = tracker.track(adv_img)

                if args.att_type == 'UA':
                    _ = trackerC.track(adv_img)
        ...
        print('({:3d}) Video: {:12s} Time: {:5.1f}s Speed: {:3.1f}fps'.format(
            v_idx+1, video.name, toc, v_idx/toc))
